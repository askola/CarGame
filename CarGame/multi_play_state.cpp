...
MultiPlayState::MultiPlayState( StateMachine& machine, sf::RenderWindow& window, bool replace )
: State{ machine, window, replace }
{

    std::cout << "MultiPlayState Init" << std::endl;
    ...
    std::string mapname = "track/mappoints.txt"; //LATAA FILUSTA!!!!
...
void MultiPlayState::pause()
{
    std::cout << "MultiPlayState Pause" << std::endl;
}

void MultiPlayState::resume()
{
    std::cout << "MultiPlayState Resume" << std::endl;
}

void MultiPlayState::update(const float dt)
{
    if (!m_raceCntDwn) {
        if (m_raceInit) {
            m_clock.restart();
        ...
        if(Cars[0]->getLap() >= 4 && Cars[1]->getLap() >= 4){
...
void MultiPlayState::draw()
{
    ...
    //Draw player 1 screen, left side
    sf::View player1View(sf::Vector2f(SCALE * Cars[0]->getBody()->GetPosition().x, SCALE * Cars[0]->getBody()->GetPosition().y), sf::Vector2f(800, 1200));
    player1View.setViewport(sf::FloatRect(0, 0, 0.5f, 1));
    m_window.setView(player1View);
    ...
    //Draw player 2 screen, right side
    
    sf::View player2View(sf::Vector2f(SCALE * Cars[1]->getBody()->GetPosition().x, SCALE * Cars[1]->getBody()->GetPosition().y), sf::Vector2f(800, 1200));
    player2View.setViewport(sf::FloatRect(0.5f, 0, 0.5f, 1));
    m_window.setView(player2View);
    ...
    miniMap.setViewport(sf::FloatRect(0.375f, 0.75f, 0.25f, 0.25f));
    ...
    timers.setViewport(sf::FloatRect(0, 0, 0.5f, 1));
    m_window.setView(timers);
    m_window.draw(m_timer);
    m_lapTimer.setString(Cars[0]->getLapFormat());
    m_window.draw(m_lapTimer);
    
    timers.setViewport(sf::FloatRect(0.5f, 0, 0.5f, 1));
    m_window.setView(timers);
    m_window.draw(m_timer);
    m_lapTimer.setString(Cars[1]->getLapFormat());
    m_window.draw(m_lapTimer);
